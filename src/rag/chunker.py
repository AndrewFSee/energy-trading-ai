...
and vector store retrieval.  Uses LangChain's RecursiveCharacterTextSplitter.
"""
...
class TextChunker:
    ...
    def _get_splitter(self):  # type: ignore[return]
        """Lazy-initialise the LangChain text splitter."""
        ...
        try:
            ...
            logger.debug(
                "RecursiveCharacterTextSplitter created (size=%d, overlap=%d)",
                self.chunk_size,
                self.chunk_overlap,
            )
            return self._splitter
        except ImportError:
            logger.error("langchain not installed — cannot create text splitter")
            raise
